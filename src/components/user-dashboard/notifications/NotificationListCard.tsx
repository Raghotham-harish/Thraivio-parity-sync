...
const NotificationListCard = ({
  ...
  return (
    <div
      className={`flex items-center gap-3 rounded-xl border border-border px-4 py-3 shadow-sm transition-colors ${
        isUnread ? "bg-secondary/40" : "bg-card hover:bg-secondary/40"
      }`}
    >
      {notification.image ? (
        <img
      ...
      {notification.actionLabel && (
        <button
          ...
          className="hidden shrink-0 text-xs font-semibold text-primary hover:underline lg:inline"
        >
          {notification.actionLabel}
        </button>
      )}
